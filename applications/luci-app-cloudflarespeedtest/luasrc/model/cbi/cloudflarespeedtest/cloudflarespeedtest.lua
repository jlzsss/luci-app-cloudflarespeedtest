...
o=s:option(Flag,"allip_enabled",translate("AllIP Enabled"))
o.description = translate("if AllIP is enabled, IPv4 and IPv6 will be tested")
o.default = 0
o.rmempty=false
...
if nixio.fs.access("/etc/config/xray_core") then
	
	o=s:taboption("xraytab", Flag, "xray_core_enabled",translate("Xray Enabled"))
	o.rmempty=true	

	local xray_core_server_table = {}
	uci:foreach("xray_core", "servers", function(s)
		if s.alias then
			xray_core_server_table[s[".name"]] = "[%s]:%s" % {string.upper(s.protocol or s.type), s.alias}
		elseif s.server and s.server_port then
			xray_core_server_table[s[".name"]] = "[%s]:%s:%s" % {string.upper(s.protocol or s.type), s.server, s.server_port}
		end
	end)

	local xray_core_key_table = {}
	for key, _ in pairs(xray_core_server_table) do
		table.insert(xray_core_key_table, key)
	end

	table.sort(xray_core_key_table)

	o = s:taboption("xraytab", DynamicList, "xray_core_services",
			translate("Xray Servers"),
			translate("Please select a service"))
			
	for _, key in pairs(xray_core_key_table) do
		o:value(key, xray_core_server_table[key])
	end
	o:depends("xray_core_enabled", 1)
	o.forcewrite = true

end

s:tab("v2raytab", translate("V2ray"))
if nixio.fs.access("/etc/config/v2ray") then
	
	o=s:taboption("v2raytab", Flag, "v2ray_enabled",translate("V2ray Enabled"))
	o.rmempty=true	

	local v2ray_server_table = {}
	uci:foreach("v2ray", "outbound", function(s)
		if s.alias then
			v2ray_server_table[s[".name"]] = "[%s]:%s" % {string.upper(s.protocol or s.type), s.alias}
		elseif s.s_vmess_address and s.s_vmess_port then
			v2ray_server_table[s[".name"]] = "[%s]:%s:%s" % {string.upper(s.protocol or s.type), s.s_vmess_address, s.s_vmess_port}
		end
	end)

	local v2ray_key_table = {}
	for key, _ in pairs(v2ray_server_table) do
		table.insert(v2ray_key_table, key)
	end

	table.sort(v2ray_key_table)

	o = s:taboption("v2raytab", DynamicList, "v2ray_services",
			translate("V2ray Servers"),
			translate("Please select a service"))
			
	for _, key in pairs(v2ray_key_table) do
		o:value(key, v2ray_server_table[key])
	end
	o:depends("v2ray_enabled", 1)
	o.forcewrite = true
